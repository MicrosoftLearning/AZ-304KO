...
- 통합된 마이그레이션 플랫폼: 하나의 포털에서 Azure로의 마이그레이션을 시작, 실행, 추적할 수 있습니다.
- 도구 범위: 평가 및 마이그레이션을 위한 다양한 도구입니다. 도구에는 Azure Migrate가 포함됩니다. 서버 평가 및 Azure Migrate: 서버 마이그레이션. Azure Migrate는 다른 Azure 서비스, 기타 도구 및 ISV(Independent Software Vendor) 오퍼링과 통합됩니다.
...
1. **az30308a-hv-vm | 네트워킹** 블레이드에서 **az30308a-hv-vm-nic1**을 선택한 다음 **인바운드 포트 규칙 추가**를 선택합니다.
...
1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션의 서버 관리자 창에서 **로컬 서버**를 클릭하고 **IE 강화된 보안 구성** 레이블 옆에 있는 **켜기** 링크를 클릭한 뒤 **IE 강화된 보안 구성** 대화 상자에서 둘 다 **끄기** 옵션을 선택합니다.

1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션에서 Internet Explorer를 시작하고 [Windows 서버 평가](https://www.microsoft.com/ko-kr/evalcenter/evaluate-windows-server-2019)를 찾은 뒤 Windows 서버 2019 **VHD** 파일을 **F:\VHDs** 폴더(이 폴더를 먼저 만들어야 함)에 다운로드합니다. 

1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션 내에서 **Hyper-V 관리자**를 시작합니다. 

1. **Hyper-V 관리자** 콘솔에서 **az30308a-hv-vm** 노드를 선택하고 **새로 만들기** 선택한 후 계단식 메뉴에서 **가상 머신**을 선택합니다. 이렇게 하면 **새 가상 머신 마법사**가 시작됩니다. 
...
1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션에서 Internet Explorer를 시작하고 [Microsoft 다운로드 센터](https://aka.ms/migrate/script/hyperv)로 이동한 후 다운로드 폴더에 PowerShell 스크립트를 다운로드하여 구성합니다.
...
1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션 내에서, Windows PowerShell ISE를 시작합니다. 

1. **관리자:**에서** Windows PowerShell ISE** 창의 콘솔 창에서 다음을 실행하여 Zone.Identifier 대체 데이터 스트림을 제거합니다. 이 경우, 파일이 인터넷에서 다운로드했음을 나타냅니다.

   ```powershell
   Unblock-File -Path C:\Users\Student\Downloads\MicrosoftAzureMigrate-Hyper-V.ps1
   ```

1. **관리자: Windows PowerShell ISE** 창에서 **다운로드 폴더**에 있는 **MicrosoftAzureMigrate-Hyper-V.ps1** 스크립트를 열고 실행합니다. 확인 메시지가 표시되면 다음 프롬프트를 제외하고(이 경우, **N** 키를 누르고 **Enter** 키를 누름) **Y** 키를 누른 후 **Enter** 키를 누릅니다.
...
1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션에서 Internet Explorer를 시작하고 [Azure Portal](https://portal.azure.com)로 이동하며 사용자 계정의 자격 증명을 이 랩에서 사용할 구독에서 소유자 역할로 제공하여 로그인합니다.

1. Azure Portal에서 **Azure Migrate**를 검색 및 선택하고 **Azure Migrate** 블레이드의 **마이그레이션 목표** 섹션에서 **서버**를 선택한 후 **도구 추가**를 선택합니다.

1. **도구 추가** 블레이드의 **마이그레이션 프로젝트** 탭에서 다음 설정을 지정한 후(다른 설정은 기본값 유지) **다음**을 선택합니다.
...
1. **도구 추가** 블레이드의 **평가 도구 선택** 탭에서 **Azure Migrate: 서버 평가를 선택한 후** **다음** 을 선택합니다.

1. **도구 추가** 블레이드의 **마이그레이션 도구 선택** 탭에서 **지금 마이그레이션 도구 추가 건너뛰기** 확인란을 선택한 후 **다음**을 선택합니다.

1. **도구 추가** 블레이드의 **검토 + 도구 추가** 탭에서 **도구 추가**를 선택합니다. 
...
1. **가상 네트워크 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정하고(나머지는 기본값을 그대로 유지) **다음: IP 주소를 선택합니다.**
...
1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션 내, Internet Explorer 창의 Azure  Portal에 있는 **Azure Migrate | 서버** 블레이드에서 **검색**(**Azure Migrate: 서버 평가** 타일에 있음)을 선택합니다. 

1. **컴퓨터 검색** 블레이드의 **컴퓨터가 가상화되었습니까** 드롭다운 목록에서 **예(Hyper-V 사용)**를 선택한 다음 **다운로드**를 선택하고 메시지가 표시되면 다운로드 위치를 **F:\VM** 폴더로 설정합니다.

    >**참고**: 다운로드가 완료될 때까지 기다립니다. 배포가 완료되려면 5분 정도 걸립니다.

1. 다운로드가 완료되면 다운로드한 .ZIP 파일의 콘텐츠를 **F:\VM** 폴더에 추출합니다. 

1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션에서 **Hyper-V 관리자** 콘솔로 전환한 후 **az30308a-hv-vm** 노드를 선택하고 **가상 머신 가져오기**를 선택합니다. 그러면 **가상 머신 가져오기** 마법사가 시작됩니다.

1. **가상 머신 가져오기** 마법사의 **시작하기 전에** 페이지에서 **다음 >**을 클릭합니다.

1. **가상 머신 가져오기** 마법사의 **폴더 찾기** 페이지에서 추출된 파일의 위치를 지정하고 **다음 >**을 선택합니다.

1. **가상 머신 가져오기** 마법사의 **가상 머신 선택** 페이지에서 **다음 >**을 선택합니다.

1. **가상 머신 가져오기** 마법사의 **가져오기 유형 선택** 페이지에서 **가상 머신 복사(새 고유 ID 만들기)**를 선택한 후 **다음 >**을 선택합니다.

1. **가상 머신 가져오기** 마법사의 **가상 머신 파일에 대한 폴더 선택** 페이지에서 **다른 위치에 가상 머신 저장** 확인란을 선택하고 각 위치를 **F:\VM** 폴더로 설정한 후 **다음 >**을 선택합니다.

1. **가상 머신 가져오기** 마법사의 **가상 머신 파일에 대한 폴더 선택** 페이지에서 **위치**를 **F:\VHD** 폴더로 설정한 후 **다음 >**을 선택합니다.
...
1. **가상 머신 가져오기** 마법사의 **요약** 페이지에서 **완료**를 선택합니다.
    ...
    >**참고**: 다운로드가 완료될 때까지 기다립니다. 배포가 완료되려면 10분 정도 걸립니다.
...
1. 가상 어플라이언스에 대한 가상 머신 연결 창의 **관리자:**에서 Windows PowerShell을 시작합니다.**Windows PowerShell** 창은 다음을 실행하여 컴퓨터 이름을 설정합니다. 
...
1. 가상 어플라이언스에 대한 원격 데스크톱 세션 내의 Windows PowerShell 콘솔에서 다음을 실행하여 Hyper-V 호스트를 나타내는 항목을 로컬 호스트 파일에 추가합니다.

   ```cmd
   Add-Content -Path C:\Windows\System32\drivers\etc\hosts -Value "10.0.2.1  az30308a-hv-vm" -Force
   ```

    >**참고**: 그러면 가상 어플라이언스와 Hyper-V 호스트 간의 이름 확인이 허용됩니다.

1. **az30308a-hv-vm**에 대한 원격 데스크톱 세션 내에서 Internet Explorer를 시작하고 [https://'IPaddress':44368](https://`IPaddress`:44368)로 이동합니다. 여기서 'IPaddress' 자리 표시자는 이전 단계에서 식별된 IP 주소를 나타냅니다.

1. 브라우저 창에서 웹 사이트의 보안 인증서에 대한 경고를 무시하고 **이 웹 사이트로 계속(권장하지 않음)** 링크를 선택합니다 
...
1. Internet Explorer 창의 **Azure Migrate에 대한 검색 설정** 페이지에서 **사용 조건 수락** 섹션의 확인란을 선택하고, 필수 구성 요소가 확인될 때까지 기다린 다음 **계속**을 선택합니다. 

1. Internet Explorer 창의 **Azure Migrate에 대한 검색 설정** 페이지에 있는 **Azure Migrate에 등록** 섹션에서 이 랩에서 사용 중인 구독의 소유자 역할을 가진 사용자 계정의 자격 증명을 제공하여 **로그인**을 선택합니다. 대상 Azure 구독 및 이 랩에서 이전에 만든 Azure Migrate 프로젝트를 선택하고 **어플라이언스 이름 입력** 텍스트 상자에 **az30308a-vma1**을 입력하고 **등록**을 선택합니다. 

1. Internet Explorer 창의 **Azure Migrate에 대한 검색 설정** 페이지에서 등록이 성공했는지 확인하고 **계속**을 선택합니다. 

1. Internet Explorer 창에서 **Azure Migrate에 대한 검색 설정** 페이지에 있는 **Hyper-V 호스트 세부 정보 제공** 섹션에서 다음 설정을 지정한 뒤, **세부 정보 저장**을 클릭합니다.

    | 설정 | 값 | 
    | --- | --- |
    | 사용자 이름 | **학생** |
    | 암호 | **Pa55w.rd1234** |
    | 식별 이름 | **az30308a Hyper-V creds** |

1. Internet Explorer 창 **Azure Migrate에 대한 검색 설정** 페이지 있는 **Hyper-V 호스트 세부 정보 제공** 섹션에서 **추가**를 선택하고, **Hyper-V 호스트 및 클러스터 목록 제공** 창에서 **az30308a-hv-vm**를 입력한 뒤, **유효성 검사**를 클릭합니다. 

1. Internet Explorer 창에서 **Azure Migrate에 대한 검색 설정** 페이지에 있는 **Hyper-V 호스트 세부 정보 제공** 섹션에서 **저장 및 검색 시작**을 선택합니다. 
...
1. **평가 속성** 블레이드에서 다음 설정을 지정(다른 설정은 기본값으로 남겨둠)하고 **저장**을 선택합니다.
...
1. **평가 서버** 블레이드로 돌아가서 **새로 만들기** 옵션이 선택되어 있는지 확인하고 그룹 이름을 **az30308a-assessment-group**으로 설정한 후 그룹에 추가할 컴퓨터 목록에서 **az30308a-vm1**을 선택하고 **평가 만들기**를 선택합니다. 
...
1. **컴퓨터 검색** 블레이드에서 다음 설정을 지정하고 **리소스 만들기**를 선택합니다(다른 설정은 기본값으로 남겨두십시오).

    | 설정 | 값 | 
    | --- | --- |
    | 컴퓨터가 가상화되어 있습니까? | **예, Hyper-V로 되어 있습니다** |
...
1. Azure Portal로 전환하고 **컴퓨터 검색** 블레이드에서 온-프레미스 Hyper-V 호스트 준비 절차의 1단계에 있는 **다운로드** 버튼을 선택하여 자격 증명 모음 등록 키를 다운로드합니다. 메시지가 표시되면 **다운로드** 폴더에 등록 키를 저장합니다.
...
1. 등록이 완료되었다는 확인을 받으면 **Azure Migrate | 서버** 블레이드로 돌아가서 **Azure Migrate: 서버 마이그레이션** 타일에서, **복제** 링크를 선택합니다. 

    >**참고**: **Azure Migrate | 서버** 블레이드를 표시하는 브라우저 페이지를 새로 고쳐야 할 수 있습니다.

1. **복제** 블레이드의 **소스 설정** 페이지에 있는 **귀하의 컴퓨터가 가상화되어 있습니까?** 드롭다운 목록에서 **예, Hyper-V 사용**을 선택하고 **다음: 가상 머신**.  

1. **복제** 블레이드의 **가상 머신** 페이지에서 다음 설정을 지정 후(다른 설정을 기본 값으로 유지함), **다음: 다음: 대상 설정**을 선택합니다.
...
1. **복제** 블레이드의 **대상 설정** 페이지에서 다음 설정을 지정하고(다른 설정은 기본값으로 남겨둠) **다음: 컴퓨팅**을 선택합니다.
...
1. **복제** 블레이드의 **컴퓨팅** 페이지에서, **Azure VM 크기** 드롭다운 목록에 **Standard_D2s_v3**이 선택되어 있는지 확인하고 **OS 유형**  드롭다운 목록에서 **Windows**를 선택한 후 **다음: 디스크**.  
...
1. 마이그레이션 상태를 모니터링하려면 **Azure Migrate**로 다시 이동하여** | 서버** 블레이드로 돌아가서 **Azure Migrate: 서버 마이그레이션** 타일에서 **복제 서버** 항목을 선택한 후 **Azure Migrate: 모니터링할 수 있습니다. | 복제 컴퓨터**에서 복제 컴퓨터 목록의 **상태** 열을 검사합니다. 상태가 **장애 조치 완료** 상태로 표시되는지 확인합니다.
